/*
 * Animated canvas-based identity graph with physics simulation and data-flow particles
 */
...
export default function OrbitalIdentityGraph() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      nodes.forEach((node, i) => {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy) + 0.1; // Add small value to prevent division by zero
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-blue-900/30 overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="flex items-center gap-2.5 px-5 py-4 border-b border-blue-900/20 bg-slate-900/50">
          ...
          <span className="text-[10px] text-blue-400/60 ml-auto">Physics-based multi-channel attribution</span>
        </div>
        <canvas
          ref={canvasRef}
          className="w-full"
